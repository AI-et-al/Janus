...
* **Executor swarm** - Runs a bounded plan (plan -> validate -> execute), writes artifacts to `janus-context/artifacts/`, and reports success/failure with cost and latency. Safety controls (allowlist, timeouts, file size caps) are configurable in `CONFIGURATION.md`.
